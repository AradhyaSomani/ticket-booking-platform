...
export default async function EventsPage({
  ...
  return (
    <div className="bg-surface min-h-screen">
      <div className="max-w-5xl mx-auto px-4 py-10">
        <h1 className="font-display text-3xl font-bold mb-1">Book your next experience</h1>
        ...
        <form className="flex flex-wrap gap-2 mb-8">
          <input name="q" defaultValue={q} placeholder="Search events"
            className="bg-bg border border-hairline rounded-md px-3 py-2 text-sm placeholder:text-muted focus:border-gold outline-none transition-colors flex-1 min-w-[160px]" />
          <input name="venue" defaultValue={venue} placeholder="Venue"
            className="bg-bg border border-hairline rounded-md px-3 py-2 text-sm placeholder:text-muted focus:border-gold outline-none transition-colors w-32" />
          <input name="maxPrice" defaultValue={maxPrice} type="number" placeholder="Max ₹"
            className="bg-bg border border-hairline rounded-md px-3 py-2 text-sm placeholder:text-muted focus:border-gold outline-none transition-colors w-28" />
          <input name="dateFrom" defaultValue={dateFrom} type="date"
            className="bg-bg border border-hairline rounded-md px-3 py-2 text-sm focus:border-gold outline-none transition-colors" />
          <button className="bg-gold text-white rounded-md px-5 py-2 text-sm font-medium hover:bg-gold-dim transition-colors">
            Search
          </button>
        </form>
